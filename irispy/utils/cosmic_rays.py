...
def _remove_cosmic_rays_rsliding(
    ...
    *,
    ...
    """
    For rsliding, there are several methods a user can select from:
    ...
    * SlidingSigmaClipping which performs a sliding sigma clipping (NaN handling done).
    ...
    The default is SlidingSigmaClipping, to select the others, pass the method name as the "method"
    key in method_kwargs, e.g. ``method_kwargs={"method": "SlidingMedian"}``.
    """
    rsliding = _import_optional_backend("rsliding", method="rsliding")
    rsliding_method = method_kwargs.pop("method", None)
    if rsliding_method is not None:
        if not hasattr(rsliding, rsliding_method):
            msg = f"Unknown rsliding method {rsliding_method!r}. Available methods: SlidingMean, SlidingMedian, SlidingStandardDeviation, SlidingSigmaClipping."
            raise ValueError(msg)
        rsliding_call = getattr(rsliding, rsliding_method)
    else:
        rsliding_call = rsliding.SlidingSigmaClipping
    ...
    clipped = rsliding_call(data=working_data, **method_kwargs).clipped
