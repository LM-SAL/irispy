...
metadata ``TWAVE`` (an explicit rest wavelength is required when it is missing), and mapped to color through ``velocity_norm``, by default an asinh scale of 25 km/s.
